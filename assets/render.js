(() => {
  ...
  const render = (data) => {
    ...
    if (site.brand) { $('#brandName').textContent = site.brand; $('#logo').textContent = site.brand.slice(0,1).toUpperCase(); }

    const repo = site.repo || site.github || '';
    const repoBtn = $('#repoBtn');
    if (repo && repoBtn) { repoBtn.href = repo; repoBtn.textContent = 'GitHub'; }
    ...
    content.innerHTML = cats.map(cat => {
      ...
      const badge = `<span class="badge ${colorClass(cat.color)}">${escapeAttr(cat.emoji || '📁')}</span>`;
